...
const AddInitialStatement = ({ onBack, onSave, mode, data, onEdit }: Props) => {
  console.log('AddInitialStatement props:', { mode, data, isView: mode === 'view', isEdit: mode === 'edit', isAdd: mode === 'add' });
  const [initialName, setInitialName] = useState(data?.provider_name || "");
  const [date, setDate] = useState(data?.statement_date ? data.statement_date.slice(0,10) : "");
  ...
  useEffect(() => {
    if (mode === 'add') {
      ...
      setDate((data.statement_date ? data.statement_date.slice(0, 10) : '') || (data.created_at ? data.created_at.slice(0, 10) : '') );
      setContact(data.contact_info || '');
      setRole(
        (data.person_role ? data.person_role.charAt(0).toUpperCase() + data.person_role.slice(1) : 'Witness') ||
        (data.type_Party ? data.type_Party.charAt(0).toUpperCase() + data.type_Party.slice(1) : 'Witness')
      );
  ...
  const handleSave = async () => {
    ...
    try {
      const payload = {
        ...
        evidence_file_path: evidenceFiles.length > 0 ? evidenceFiles : undefined,
  ...
  return (
    <div className="bg-white rounded-xl shadow border max-w-3xl mx-auto mt-8 mb-8">
      <div className="bg-[#e9f1fa] px-6 py-4 rounded-t-xl flex justify-between items-center">
        ...
        {isAdd ? 'ADD INITIAL STATEMENT' : isEdit ? 'EDIT INITIAL STATEMENT' : 'VIEW INITIAL STATEMENT'}
        </h2>
      </div>
      <div className="p-6">
        ...
        <div className="mb-6">
          ...
          <div className="flex gap-2 mb-2">
            <input
              ...
              disabled={isView}
            />
            {!isView && <button className="px-4 py-2 rounded bg-gray-300" onClick={() => setStatement("")}>Cancel</button>}
            {!isView && <button className="px-4 py-2 rounded bg-blue-600 text-white" onClick={() => setStatement(statement)}>Add</button>}
          </div>
          <textarea
        ...
        <div className="mb-6">
          ...
          <div className="flex gap-4 flex-wrap">
            {evidenceFiles.map((file, idx) => (
              <div key={file.public_id || idx} className="bg-[#f6f7f8] border rounded p-4 w-48 flex flex-col items-center relative">
                <a href={file.file_url} target="_blank" rel="noopener noreferrer" className="text-red-500 font-bold mb-1">{file.original_name}</a>
