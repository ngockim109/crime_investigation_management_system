...
const SceneMediasTable = ({ data, onEdit, onDelete, onView }: SceneMediasTableProps) => (
      ...
      {(data ?? []).map((item, idx) => (
        <tr key={item.id} className="border-b">
          ...
          <td className="p-2 border w-30">
            {Array.isArray(item.scene_media_file) && item.scene_media_file.length > 0 ? (
              <div className="flex flex-col gap-1">
                {item.scene_media_file.map((file: any, fileIdx: number) => (
                  file.file_url ? (
                    ...
                    <span key={fileIdx} className="text-gray-400">—</span>
                  )
                ))}
              </div>
            ) : (
          ...
          <td className="p-2 border w-52">{item.description || item.scene_media_description || '—'}</td>
          <td className="p-2 border w-25">{formatDateTime(item.date || item.date_taken)}</td>
