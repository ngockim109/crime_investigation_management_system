...
import { addMedicalSupport, resetInitialResponse } from '@/redux/reduxInitialResponse'
...
const MedicalRescueSupport = () => {
    ...
    const handleAddSupport = () => {
        try {
            const existsIndex = supports.findIndex(
                (s) => s.medical_unit_id === dataForm.medical_unit_id
            )
            if (existsIndex !== -1) {
                // Update existing
                ...
                toast.success("Support information sent successfully")
            }
            setShowDialog(false)
            resetForm()
        } catch (err) {
    ...
    const handleDeleteSupport = (medical_unit_id: string) => {
        setSupports((prev) => prev.filter((s) => s.medical_unit_id !== medical_unit_id))
    }
            ...
            .then((v) => {
                setDataForm({
                    ...dataForm, scene_sketch_file:
                        [...dataForm.scene_sketch_file, { ...v.data[0], original_name: file.name }]
                })
            })
            .finally(() => {
                setLoading(false)
        ...
        () =>
            supports.map((unit) => (
                <TableRow key={unit.medical_unit_id} className="border-none">
                    ...
                    <TableCell className="py-2 px-4">{unit.time_of_arrival}</TableCell>
                    <TableCell className="py-2 px-2 flex gap-2">
                        ...
                        <Button size="icon" variant="ghost" onClick={() => handleDeleteSupport(unit.medical_unit_id)}>
                            <Trash2 size={14} />
                        </Button>
                    </TableCell>
                </TableRow >
                ...
                <div className="flex justify-between items-center mb-2">
                    ...
                    <Dialog open={showDialog} onOpenChange={setShowDialog}>
                        <DialogTrigger asChild>
                            <Button variant="outline" size="sm" className="bg-blue-100 p-2" onClick={() => { resetForm() }}>
                        ...
                        <DialogContent className="min-w-[50%] max-h-screen  overflow-y-auto">
                            ...
                            <div className="space-y-4">
                                <div className='flex flex-col gap-4'>
                                    ...
                                    <Input value={dataForm.medical_unit_id} onChange={(e) => setDataForm({ ...dataForm, medical_unit_id: e.target.value })} />
                                </div>
                                <div className="flex flex-col gap-4">
                                    <label className="font-medium">Type of Support Provided</label>
                                    <Input value={dataForm.support_type} onChange={(e) => setDataForm({ ...dataForm, support_type: e.target.value })} />
                                </div>
                                <div className="flex justify-between">
                                    <div className='flex flex-col gap-2'>
                                        <label className="font-medium">Personnel Assigned</label>
                                        <Input value={dataForm.personnel_assigned} onChange={(e) => setDataForm({ ...dataForm, personnel_assigned: e.target.value })} />
                                    </div>
                                    <div className='flex flex-col gap-2'>
                                        ...
                                        <Input
                                            ...
                                            onChange={(e) => setDataForm({ ...dataForm, time_of_arrival: e.target.value })}
                                ...
                                <div className="flex flex-col gap-4">
                                    ...
                                    <Input value={dataForm.location_assigned} onChange={(e) => setDataForm({ ...dataForm, location_assigned: e.target.value })} />
                                </div>
                                <div className="flex flex-col gap-4">
                                    <label className="font-medium">Remarks/Notes</label>
                                    <Textarea
                                        value={dataForm.remarks}
                                        onChange={(e) => setDataForm({ ...dataForm, remarks: e.target.value })}
                                ...
                                <div className="space-y-2">
                                                ...
                                                <div className="flex flex-wrap">
                                                    {dataForm.scene_sketch_file.map((v, i) => (
                                                        <FileForm
                                                            ...
                                                            rm={() => setDataForm({
                                                                ...dataForm,
                                                                scene_sketch_file: dataForm.scene_sketch_file.filter((_, index) => index !== i)
                                                            })}
                                                        />
                                                    ))}
                                                </div>
                                    ...
                                    <Attachments key={dataForm.scene_sketch_file.length} idimage="RelevantPartiesForm" onchange={(file) => {
                                        setFile(file)
                                    }} />
                                </div>
