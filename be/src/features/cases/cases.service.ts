...
export class CasesService {
  ...
  async createCaseFromReport(report: Report, manager: EntityManager): Promise<Case> {
    try {
      const caseData = {
  ...
  async getAllCasesWithReports(filter: GetCasesFilterDto): Promise<PaginatedCasesResult> {
    const {
      search,
      status,
      type_of_crime,
    ...
    if (level_of_severity) {
      query.andWhere('case.severity = :level_of_severity', { level_of_severity });
    }
  ...
  async findCaseById(caseId: string): Promise<InitialResponseDetail> {
    try {
      ...
      if (!foundCase || !foundCase.initial_response) {
        throw new NotFoundException(`Initial response for case ID ${caseId} not found`);
      }

      return plainToInstance(InitialResponseDetailDto, foundCase.initial_response, { excludeExtraneousValues: true });
    } catch (error) {
      throw error;
    }
  }
