...
export class EvidenceService {
  ...
  async createEvidence(createEvidenceDto: CreateEvidenceDto): Promise<Evidence> {
    ...
    try {
      ...
      throw new InternalServerErrorException('Failed to create evidence', error.message);
    }
  }
  ...
  async findAllEvidence(): Promise<Evidence[]> {
    ...
    try {
      ...
      throw new InternalServerErrorException('Failed to get all evidence', error.message);
    }
  }
  ...
  async findOneEvidence(evidence_id: string): Promise<Evidence | null> {
    ...
    try {
      ...
      this.logger.error(`Failed to get evidence with id: ${evidence_id}`, error.stack);
      if (error instanceof NotFoundException) throw error;
      throw new InternalServerErrorException('Failed to get evidence', error.message);
    }
  }

  async uploadMultipleFiles(files: Multer.File[]): Promise<Attachment[]> {
    
    // this.logger.log(`Uploading ${files?.length || 0} files for evidence via HTTP`);
    try {
      return await this.fileUploadService.uploadMultipleFiles(files, 'evidence');
    ...
    //   return response.data;
      ...
      this.logger.error('Failed to upload files via HTTP', error.stack);
      throw new InternalServerErrorException('Failed to upload files', error.message);
    }
  }

  async updateEvidence(evidence_id: string, updateDto: Partial<Evidence>): Promise<Evidence> {
    this.logger.log(`Updating evidence with id: ${evidence_id}`);
    try {
      const evidence = await this.evidenceRepository.findOneBy({ evidence_id });
      if (!evidence) {
        this.logger.warn(`Evidence not found for update with id: ${evidence_id}`);
        throw new NotFoundException('Evidence not found');
      }
      ...
      this.logger.error(`Failed to update evidence with id: ${evidence_id}`, error.stack);
      if (error instanceof NotFoundException) throw error;
      throw new InternalServerErrorException('Failed to update evidence', error.message);
    }
  }
  ...
  async deleteEvidence(evidence_id: string): Promise<void> {
    ...
    try {
      const evidence = await this.evidenceRepository.findOneBy({ evidence_id });
      if (!evidence) {
        this.logger.warn(`Evidence not found for delete with id: ${evidence_id}`);
        throw new NotFoundException('Evidence not found');
      }
      ...
      this.logger.error(`Failed to delete evidence with id: ${evidence_id}`, error.stack);
      if (error instanceof NotFoundException) throw error;
      throw new InternalServerErrorException('Failed to delete evidence', error.message);
    }
  }
} 